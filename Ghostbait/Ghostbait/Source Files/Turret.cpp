...
void Turret::Update() {
	...
	if(target != nullptr) {
		...
		DebugRenderer::AddLine(newpos, (XMFLOAT3)target->GetPosition().m[3], (XMFLOAT3)turretPitch->m[3]);
		XMVECTOR bulletpos = DirectX::XMLoadFloat4(&(XMFLOAT4)turretPitch->m[3]);
		XMVECTOR targetPos = DirectX::XMLoadFloat4(&(XMFLOAT4)target->GetPosition().m[3]) + XMVectorSet(0,1,0,0);
